...
class _EmployeeCampaignDetailScreenState extends State<EmployeeCampaignDetailScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final overview = await employeeApiClient.getCampaignLive(widget.campaign.id);
      if (!mounted) return;
  ...
  Widget _buildBody(bool ar) {
    ...
    // Mobile Recovery + Current-Backend Alignment (2026-09-15): the
    // current GET /company/campaigns/:id/live payload is
    // { funnel: {entered, eligible, trialRedeemed, surveyComplete}, ...,
    //   purchaseIntent: {averageScore, responses} } — not the old
    ...
    final surveyComplete = funnel['surveyComplete'] as int? ?? 0;
    final completionRate = entered > 0 ? ((surveyComplete / entered) * 100).round() : 0;
    final avgScore = purchaseIntent['averageScore'];
    final stats = <(String, String)>[
      (ar ? 'التجارب المستردة' : 'Trials Redeemed', '${funnel['trialRedeemed'] ?? 0}'),
      (ar ? 'الاستبيانات المكتملة' : 'Survey Completions', '$surveyComplete'),
      (ar ? 'نسبة الإتمام' : 'Completion Rate', '$completionRate%'),
      (ar ? 'متوسط نية الشراء' : 'Avg. Purchase Intent', avgScore != null ? '$avgScore/5' : '—'),
    ];
