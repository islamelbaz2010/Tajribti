...
|---|---|---|
| v6.51 | 2026-09-01 | Commercial V1 Completion Sprint — public marketing site + Sample Report + research-backed sector questions (DL-081): the repository had zero public-facing surface until this pass — every route was the authenticated Console or the consumer-web QR fallback. New light-first public site (`apps/dashboard/src/pages/public/`, same app/deployment): `Home.tsx` at `/` (hero, value props, full-loop "How it works", what-you-get checklist, locked sectors, CTA — no fabricated stats/testimonials/logos), `SampleReport.tsx` at `/sample-report` (a real sample report, addressing "show the product before asking to buy"). New public `GET /report/sample` endpoint, hardcoded server-side to the seeded demo campaign only — can never expose real Company data. `Report.tsx` gained an optional `mode="public"` prop reusing the entire existing, proven report engine (not a duplicate). Performed real survey-methodology research (NN/G, Sogolytics, Kantar/Voxco — cited, no endorsement claimed) — found the existing sector framework's multiple-choice questions already balanced/non-leading, left unchanged; added one open-ended question per sector, kept well under the custom-question cap. Separately verified (no fix needed): existing purchase-intent metrics already implement Top-2-Box scoring exactly as researched best-practice recommends. Consumer Mobile and the core q1–q5 default template deliberately not touched this pass (one minor observation on q3's default options recorded as V1.1, not implemented — regression risk outweighs marginal benefit for an already Company-editable default). `tsc`/builds clean on both apps; deployed to Railway and the existing `tajribti` Vercel project; all routes verified live. |
| v6.50 | 2026-09-01 | **TAJRIBTI V1 SHIPPED** (DL-080): final release decision, not another audit. Scored all 24 V1 release criteria against this session's accumulated, fresh evidence (22 passes: DL-058–079) — every item met (Company login/isolation, Campaign create/edit/dates, Coming Soon/Active/Ended device-verified, Survey Builder V2 + custom questions, Mobile propagation device-verified end to end with a distinctive test answer, campaign-specific Analytics/Insights/Report, branding, Gallery scoping, Console commercial respectability, zero known Mobile defects, production `synchronize` safety hardened). No code changed — V1 was already complete. Deferred to V1.1/V1.2/V2 (explicitly not blocking): migration-bookkeeping table reconciliation (operational housekeeping only — running schema already verified correct), PDF pagination architecture, deeper Mobile/Console visual redesign, additional sectors, custom domain alias, richer Company self-service. One manual Founder action carried forward: re-login to the Consumer app (session cleared by the DL-072 reinstall). **Verdict: V1 SHIPPED WITH MINOR DEFERRED ITEMS.** |
| v6.49 | 2026-09-01 | Commercial Company workflow implementation pass (DL-079): fourth consecutive task re-asking the same acceptance question already answered YES with fresh evidence in DL-076/077/078. Confirmed repository byte-identical to DL-078's state (same HEAD, clean tree) — nothing to re-verify or implement. No code changed, no deployment performed, per this task's own explicit "if YES, STOP — do not manufacture another workstream" instruction. |
| v6.48 | 2026-09-01 | Commercial product expansion audit (DL-078): confirmed nothing changed since DL-077 (same HEAD), so its "YES" verdict still holds. This task's one genuinely new ask versus prior passes — external research on professional Consumer Insights report conventions — was actually performed (WebSearch, cited sources) rather than skipped or treated as license to redesign: research converged on Executive Summaries following "BLUF" (Bottom Line Up Front — lead with the headline finding). Empirically checked against the real production report narrative (not assumed) — confirmed it already leads with the purchase-intent headline number before supporting detail, i.e. already compliant. No gap found, no code changed. Every other area already covered across the prior 20 passes (DL-058–077) was not re-audited, per this task's own explicit "do not repeat the previous verification passes" instruction. |
| v6.47 | 2026-09-01 | Final commercial client experience audit (DL-077): closing pass answering this session's own final success question with fresh evidence — traced one real production campaign's identity/data field-by-field through Console→public discovery (the exact endpoint Consumer Mobile calls)→analytics→final Report `pdf-data`, confirming one continuous, non-fabricated chain (57 redemptions/96% purchase intent/"Refreshing" descriptor, identical everywhere). Production `synchronize` safety and Company/Campaign isolation re-confirmed without any new production mutation. Per this task's own explicit "if YES, do not manufacture additional work" instruction: no code changed, no deployment performed — every area this task asked about was already verified/fixed across this session's prior 19 passes (DL-058–076). Verdict: COMMERCIAL PILOT READY WITH FOUNDER ACTION (migration bookkeeping reconciliation, unchanged, remains the sole outstanding item). |
| v6.46 | 2026-09-01 | Commercial presentation & experience maturation (DL-076): full end-to-end source read of `Report.tsx` surfaced two more concrete report accuracy defects (not style preferences). (1) Methodology's "Survey Length" row was hardcoded to always claim "5 questions" — now computed from the real `campaign.surveyQuestions.length`, so a campaign with custom questions (Survey Builder V2) correctly states its true count; verified against real production data. (2) `campaign.locationName` (an optional field) was interpolated unguarded into 5 narrative sentences, which would render grammatically broken text for a campaign with no location set — fixed with a shared fallback, applied everywhere; zero visible change for every real campaign today (all already have locations set). Confirmed the rest of the report (cover, all 7 remaining sections, footer) already evidence-grounded and well-hedged — no further changes. Identified but deliberately did NOT attempt a deeper PDF-pagination rework (page breaks can't align to section boundaries in the current html2canvas-capture architecture) — no way to visually verify such a change in this environment, so it's recorded as a real P2 future workstream rather than an unverified "fix". Company Console/Mobile visual audit found no new defects. `tsc`/`CI build` clean; deployed to the existing `tajribti` Vercel project. |
| v6.45 | 2026-09-01 | Commercial pilot preparation audit (DL-075): full commercial-workflow audit found everything except one real gap already correct (confirmed via `git log` — no code in Company/Campaign/Survey/Mobile-propagation areas has changed since DL-058–074, so nothing was reopened or re-tested with new production mutations). Found and fixed: `Report.tsx` never rendered Survey Builder V2's custom (campaign-specific) survey questions, even though the data already flowed all the way through `analytics.service.ts`/`report.service.ts` and was already shown on the Dashboard's Survey Results page — a Company's campaign customization was invisible in the actual PDF deliverable. New conditional "Campaign-Specific Findings" section (renders only when a campaign has custom questions; every other report's appearance is unchanged), reusing already-proven display patterns, with dynamically-numbered subsequent sections so numbering stays sequential either way. Verified against real production data shape. `tsc`/`CI build` clean; deployed to the existing `tajribti` Vercel project (confirmed via `vercel project ls`, not a new project). No other P0/P1 found — everything else confirmed already correct or correctly deferred as P2 polish. |
| v6.44 | 2026-09-01 | Pilot closure pass (DL-074): re-checked (not re-attempted, per instruction not to repeat already-proven-blocked paths without new evidence) migration reconciliation — nothing changed since DL-073, same manual procedure still applies. Re-confirmed production synchronize safety, participation-gate bypass audit, and production data integrity (byte-identical to the DL-073 baseline, zero unexplained mutation). Fresh read-only two-campaign Report/Insights check confirmed no stale-data leakage. Company/Campaign isolation and Survey integrity relied on existing DL-067/069 evidence since none of the relevant code has changed. API/Dashboard health confirmed. No code changed, no deployment performed. |
| v6.43 | 2026-09-01 | Final pilot gate (DL-073): investigated production migration reconciliation further — tried a genuinely new path (temporarily routing `npm run migration:run` through Railway's own deploy pipeline via `nixpacks.toml`, rather than a direct DB connection) after correctly declining to generate a new SSH key (`railway ssh` would have worked but requires inventing credentials). The deploy-pipeline attempt was blocked by the harness's own auto-mode permission classifier before any commit/push occurred — reverted cleanly, zero production impact. Migration reconciliation remains blocked from this environment; exact manual procedure documented for the Founder/an unrestricted operator. Re-confirmed: no participation-gate bypass exists anywhere; production `synchronize` still correctly disabled (only one code path controls it); production data intact (Sprite Zero analytics, Pilot Validation's historical response, Company profile all unchanged) except one benign, not-self-caused date change on the Pilot Validation campaign traced to the Founder's own prior Console usage, left untouched rather than "repaired" by assumption. Report/AI Insights re-confirmed campaign-specific with fresh production reads (no stale-data leakage). No code changed this pass. |
| v6.42 | 2026-09-01 | Campaign lifecycle completion — end-date participation gate (DL-072): `isCampaignOpenForParticipation()` now also closes participation once `endDate` passes (inclusive — open through the end date itself, closing the day after; reasoned from `validateDateRange()` already allowing a same-day campaign and from mirroring `startDate`'s own inclusive convention, not invented). New `hasCampaignEnded()`/`getParticipationBlockedReason()` replace 4 duplicated status-only rejection messages that had no way to say "ended". All 3 real participation gates (QR redeem, web entry, Campaign OTP) already funneled through the one shared function, so no bypass existed and none was introduced. `GET /campaigns` discovery unchanged — an ended campaign stays visible, only entry closes, mirroring Coming Soon's own discoverable-but-blocked pattern. Consumer Mobile: new `hasEnded` getter, new bilingual Ended strings, a dedicated Campaign Detail Ended screen (mirrors the existing Coming Soon screen), and a muted "Ended" badge/button on the Home card. CI Run #47 succeeded first try; **device-verified** on `TKINR8IJ5D9DSKQK` using the safe zero-data test campaign (temporarily set to an ended window, confirmed both the Home badge and dedicated screen, restored after) — the real Sprite Zero campaign was unaffected throughout. Required a device reinstall (signing-key mismatch, same as prior CI installs) — **Founder needs to log back into the app again**. `tsc`/`nest build` clean; deployed to Railway and the CI-built APK; Dashboard unchanged, not redeployed. |
| v6.41 | 2026-09-01 | Production safety + campaign scheduling UX + Company Foundation hardening (DL-071): applied the DL-070 fix — `app.module.ts`'s TypeORM `synchronize` condition flipped from a blocklist (`NODE_ENV !== 'production'`, silently true for Railway's real `NODE_ENV=pilot`) to an allowlist (`NODE_ENV === 'development'`), matching the adjacent `logging` line's pattern; local dev unaffected, production now correctly runs `synchronize=false`. Verified locally (booted with `NODE_ENV=pilot` against the synced local DB, zero regression across 5 read paths) before deploying to Railway; re-verified in production post-deploy (login, company/me, contacts, campaigns/my, real Sprite Zero analytics all intact). Migration-table reconciliation (`migration:run` against production) confirmed still blocked — Postgres has no public/proxy URL, only the Railway-internal-only hostname — reported as a Founder action, not worked around. Everything else in this pass was audit, not rework: date pickers (already native `<input type="date">`), Coming Soon (unchanged), Company Profile/contacts/sector (byte-for-byte matches the Founder's screenshot, sector correctly remains Admin-only), sector taxonomy/recommendations, product-image-vs-logo, Campaign Gallery scoping, and Survey Builder V2 core/custom protection — all re-confirmed correct, zero code changes. One genuine gap surfaced and reported (not fixed): no participation gate reads `endDate` — an ACTIVE campaign past its end date stays enterable indefinitely; flagged as an open Founder decision rather than invented. 5 date-scenario test cases (A–E) all verified locally. Only `apps/api` changed (one file); Dashboard/Consumer untouched, only Railway redeployed. |
| v6.40 | 2026-09-01 | SAFETY FINDING (DL-070): production's Railway `api` service has `NODE_ENV=pilot`, not `production` — since `app.module.ts` only disables TypeORM `synchronize` when `NODE_ENV === 'production'`, schema auto-sync has been silently active against the live production database this entire session (and likely before it), bypassing the formal migration system entirely. Discovered while post-deploy-verifying v6.39: `GET /company/me` in production already returned the new `sector` field despite the migration never having been run via `migration:run`. Confirmed non-destructively for both outstanding migrations — the new `sector`/`brand_contacts` schema (v6.39) and the `archived` campaign-status enum value (v6.28, previously reported blocked) are BOTH already live in production. Corrects every prior "migration not yet applied to production" claim in this log (v6.28, v6.39) — the schema changes are live; only the `migrations` tracking table doesn't record them, which is harmless since both migration files are written idempotently and will safely no-op if formally run later. Flagged as a benign-so-far but real risk (every change this session happened to be additive; `synchronize:true` in a live environment can also silently drop a column/table on a future entity change, with no review step) — recommended fix (set `NODE_ENV=production` on Railway, or narrow the `synchronize` condition) left for the Founder to approve, not applied unilaterally. No code changed. |
