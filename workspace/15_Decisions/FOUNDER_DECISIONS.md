...
| DL-093 | 2026-09-02 | PROD | LOCKED | **LIVE PARTICIPANT COUNT / PROGRESS ON CAMPAIGN LIST VIEWS**: a third same-day benchmark task went looking for a genuine additional gap rather than repeating DL-092's report. Found one — `GET /campaigns/my` and `GET /admin/campaigns` showed campaign status only, with no participant/progress visibility without clicking into each campaign, contradicting this task's own explicit "HOW MANY PARTICIPANTS? WHAT IS THE PROGRESS?" requirement for the Admin operator view. Added one shared, grouped-`COUNT` aggregation (`CampaignService.attachParticipantCounts()`, reused by both the Company and Admin campaign-list endpoints — no duplicated logic) and rendered it as "`N / target` participants" + a progress bar on the Company's campaign cards and a new "Participants" column on Admin's campaign table. Verified with real data: a fresh campaign showed `participantCount=0`, then 3 real redemption rows raised it to 3 on both surfaces identically; all test data deleted after. Re-confirmed, not re-litigated: the three items DL-092 already correctly declined (campaign lifecycle/wizard — blocked by LOCKED DL-090; audience/eligibility enforcement; in-app support contact) still stand for the same reasons. Full detail in `DECISION_LOG.md` DL-093. | Ships the one genuine, evidence-backed capability gap this task's own priority list named, instead of either repeating an unchanged prior conclusion verbatim or inventing scope to appear more thorough than the evidence supports. | Founder / Project Director (explicit direction, this session, dated 2026-09-02) / Claude Code (session) |
| DL-094 | 2026-09-02 | PROD | LOCKED | **CAPACITY/PROGRESS ON CAMPAIGN DETAIL OVERVIEWS + RIGOROUS DL-090 RE-VERIFICATION**: a fourth same-day task explicitly demanded exact-citation proof before accepting DL-090 as a real blocker. Re-verified: DL-090 is `LOCKED`, dated 2026-09-02, recorded in this constitutional document itself (this table, DL-090's own row) as well as `DECISION_LOG.md`, explicitly declining "a change to tested commercial behavior (campaign-creation default status) without further Founder sign-off" — no later entry reverses it, so it still stands and only that specific change remains stopped, per this task's own instruction. Separately, found and fixed a genuine additional benchmark gap: neither Company Console's `Overview.tsx` nor Admin's `AdminCampaignDetail.tsx` Overview tab showed redemption count in the context of the campaign's own target (`targetCount`), despite both already having that data — directly answers this task's "WHAT IS THE CAPACITY? HOW FAR HAS IT PROGRESSED?" requirement. Both now show "N of/vs. target" plus a progress bar, reusing existing data, zero new endpoints. Full detail in `DECISION_LOG.md` DL-094. | Provides the exact-citation evidence this task demanded rather than a repeated assertion, and extends DL-093's list-view progress fix to the two campaign detail Overview screens it didn't yet cover. | Founder / Project Director (explicit direction, this session, dated 2026-09-02) / Claude Code (session) |
| DL-095 | 2026-09-02 | PROD | LOCKED | **ADMIN OPERATIONAL VISIBILITY — DATES + QR/JOIN LINK; FULL CHAIN RE-VERIFIED WITH FRESH DATA**: found, by direct inspection, that Admin's selected-campaign workspace (`AdminCampaignDetail.tsx`) showed zero campaign dates and had no QR/join-link visibility at all — the exact operational blindness that made DL-091's "Details & QR" defect hard to diagnose from the Admin side. Added a start/end date line to the header and a QR card to the Overview tab, both reusing already-fetched data / the already-public QR endpoint, no new backend surface. Re-verified the complete Campaign→Participant→Redemption→Survey→Analytics→Insights→Report chain end-to-end with a brand-new Company/campaign/consumer/survey response (not the long-reused demo fixture) — every stage returned correct, connected, real numbers, and the AI narrative generated fresh was genuinely benchmark-quality (WHO, descriptor, hedged sample-based intent, a "consider..." recommendation, bilingual). All test data deleted after. Full detail in `DECISION_LOG.md` DL-095. | Ships the one genuinely material operational-visibility gap this task named, and answers its explicit "verify the complete chain with real data" instruction with fresh, non-seed evidence. | Founder / Project Director (explicit direction, this session, dated 2026-09-02) / Claude Code (session) |
| DL-096 | 2026-09-02 | PROD | LOCKED | **COMPANY CAMPAIGN WORKSPACE PROGRESS + OPERATIONAL-EXCEPTION BANNERS CONNECTED TO THE SELECTED CAMPAIGN**: found the Company Console's own primary campaign-management screen (`CampaignDetail.tsx`) still showed a static `Target: N participants` label with no live progress — fixed to `${participantCount}/${targetCount}`, reusing DL-093's already-fetched data. Separately, the "needs attention" (active-but-expired) signal on both list views vanished the instant an operator opened the specific campaign it was about — added an identical banner + a confirm-gated "Mark Completed" quick action to both `AdminCampaignDetail.tsx` and `CampaignDetail.tsx`. Verified with a real, freshly-created, genuinely-expired campaign: confirmed a real participation attempt against it correctly 403'd (proving the banner's claim is true, not cosmetic) and the exact PATCH the button sends correctly transitions status; test data deleted after. Full detail in `DECISION_LOG.md` DL-096. | Connects two already-existing list-level signals to the one screen an operator/Company actually acts from, verified end-to-end with real data rather than trusted by inspection. | Founder / Project Director (explicit direction, this session, dated 2026-09-02) / Claude Code (session) |
