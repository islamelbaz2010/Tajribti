...
class _EmployeeLoginScreenState extends State<EmployeeLoginScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
      // Mobile Recovery + Current-Backend Alignment (2026-09-15):
      // POST /company/auth/login returns { token, employee: { id, name,
      // email, companyId } } — no companyName. A follow-up
      // GET /company/profile (same call the token is stored for) fetches
      // it once, right after login, so the Home screen header never has
      // to guess it.
      ...
      final companyId = employee['companyId'] as String? ?? '';
      ...
      String companyName = '';
      try {
        final profile = await employeeApiClient.getCompanyProfile();
        companyName = profile['name'] as String? ?? '';
      } catch (_) {
        // Non-fatal: Home screen shows a blank company name until the
        // next successful profile fetch rather than blocking sign-in.
      }
      await EmployeeAuthService.saveSession(
        token: token,
        employeeId: employeeId,
