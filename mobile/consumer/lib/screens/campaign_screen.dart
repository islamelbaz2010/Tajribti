...
class _CampaignScreenState extends State<CampaignScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      // getConsumerProfile()/checkEligibility() pre-checks have no current
      // equivalent (no profile endpoint; the current eligibility endpoint
      // is a POST with real side effects — it creates the Participation
      // row — so it must not be called just to "peek" on page load, only
      // when the consumer actually starts the trial in _start() below).
      // "Already completed" here is instead read from the consumer's real
